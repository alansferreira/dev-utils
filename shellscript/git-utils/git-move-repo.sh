...
read -p "$(bgreen "INPUT"): Enter $(byellow "destination git") ssh uri : " new_destination
...
echo -e "$(bblue "INFO"): Repository will be mirrored from \"$(bred $current_origin)\" to \"$(byellow $new_destination)\"."
echo -e "$(bblue "INFO"): Current git directory \"$(bred $git_root_path)\" and"
echo -e "         will be renamed to \"$(byellow $new_git_root_path)\"."

echo 
echo -e "$(bred "WARN"): From this point the confirmation of nexts actions $(bred "CAN NOT BE UNDONE"), then: "
echo -e "      * check previous messages."
echo -e "      * check if \"source\" and \"destination\" repositories are correct."
echo 

read -p "$(bgreen "INPUT"): Are you sure to proceded [y or n]? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Nn]$ ]]
then
    # do dangerous stuff
    echo -e "$(bblue "INFO"): Operation canceled!"
    exit 0
fi

echo -e "$(byellow "INFO"): Begin operations..."
